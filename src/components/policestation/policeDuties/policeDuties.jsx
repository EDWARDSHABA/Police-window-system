...
export default function PoliceDuties() {
  ...
  useEffect(() => {
    ...
    syncDuties();
    window.addEventListener("storage", syncDuties);

    return () => window.removeEventListener("storage", syncDuties);
  }, []);

  const dutyTypes = useMemo(
    () => ["All Duty Types", ...new Set(duties.map((row) => row.dutyType).filter(Boolean))],
    [duties]
  );
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex w-full flex-col items-stretch gap-2 sm:max-w-xs">
              <button
                type="button"
                onClick={() => navigate("/assign-duties/new")}
                className="h-12 rounded-sm bg-sky-500 px-5 text-left text-xl font-medium text-white shadow-sm transition hover:bg-sky-600"
              >
                New Duties
              </button>
                  ...
                  {filteredDuties.length === 0 ? (
                    <tr>
                      <td colSpan="6" className="px-4 py-10 text-center text-sm text-slate-500">
                        No assigned duties yet. Create one from New Duties.
                      </td>
                    </tr>
                  ) : (
