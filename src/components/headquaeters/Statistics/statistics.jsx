        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col gap-3 mb-6">
            ...
            <p className="text-gray-600 max-w-2xl">
              Analyze case flow across each registered police station by month, solved status, and case type distribution.
            </p>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow p-6 mb-6">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-4 mb-6">
              <div>
                <h2 className="text-xl font-semibold text-gray-800">Select Station</h2>
                <p className="text-sm text-gray-500">View monthly case trends and performance for a single station.</p>
              </div>
              <select
            ...
            <div className="grid gap-6 lg:grid-cols-[2fr_1fr]">
              <div className="bg-slate-50 rounded-3xl p-5 border border-slate-200">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-lg font-semibold text-gray-800">Monthly Case Trend</h3>
                    <p className="text-sm text-gray-500">Total and solved case volumes over the year.</p>
                  </div>
                ...
                <div className="overflow-x-auto">
                  <svg viewBox="0 0 700 320" className="w-full h-[320px]">
                    ...
                    {Array.from({ length: 6 }).map((_, index) => {
                      ...
                      return (
                        <g key={index}>
                          ...
                          <text x="24" y={y + 4} fontSize="12" fill="#475569">{Math.round(maxChartValue - (maxChartValue / 5) * index)}</text>
                        </g>
                      );
                    })}
                    ...
                    {currentStationData.total.map((value, index) => {
                      const x = 50 + index * 52;
                      const yTotal = 260 - (value / maxChartValue) * 220;
                      const ySolved = 260 - (currentStationData.solved[index] / maxChartValue) * 220;
                      return (
                        <g key={index}>
                          <circle cx={x} cy={yTotal} r="4" fill="#0f172a" />
                          <circle cx={x} cy={ySolved} r="4" fill="#f97316" />
                        </g>
                      );
                    })}
                    ...
                    {monthLabels.map((month, index) => {
                      const x = 50 + index * 52;
                      return (
                        <text key={month} x={x} y="295" textAnchor="middle" fontSize="11" fill="#475569">
                          {month}
                        </text>
                      );
                    })}
                ...
                <div className="grid grid-cols-2 gap-4 mt-5">
                  <div className="rounded-2xl bg-white p-4 border border-slate-200">
                    <p className="text-sm text-gray-500">Total Cases This Year</p>
                    <p className="text-2xl font-semibold text-slate-900">{totalSum}</p>
                  </div>
                  <div className="rounded-2xl bg-white p-4 border border-slate-200">
                    <p className="text-sm text-gray-500">Solved Cases This Year</p>
                    <p className="text-2xl font-semibold text-orange-500">{solvedSum}</p>
                  </div>
                </div>
              </div>

              <div className="bg-slate-50 rounded-3xl p-5 border border-slate-200">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">Case Types Distribution</h3>
                <div className="flex flex-col items-center gap-6">
                  <div
                    className="relative h-72 w-72 rounded-full"
                    style={{
                      background: `conic-gradient(${caseTypes
                        .map((type) => `${type.color} ${type.value * 3.6}deg`)
                        .join(", ")})`,
                    }}
                  >
                  ...
                  <div className="w-full">
                    {caseTypes.map((type) => (
                      <div key={type.label} className="flex items-center justify-between gap-3 mb-3">
                        <div className="flex items-center gap-3">
          ...
          <section className="bg-white rounded-3xl shadow p-6">
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-5">
              <div>
                <h2 className="text-xl font-semibold text-gray-900">Rates Across Stations</h2>
                <p className="text-sm text-gray-500">Solved vs unsolved performance for the key registered stations.</p>
              </div>
            </div>

            <div className="space-y-5">
              {stationRates.map((station) => (
                <div key={station.name} className="rounded-3xl bg-slate-50 p-4 border border-slate-200">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <p className="text-base font-semibold text-gray-800">{station.name}</p>
                      <p className="text-sm text-gray-500">Solved: {station.solved}% — Unsolved: {station.unsolved}%</p>
                    </div>
                    <div className="flex gap-2 text-sm text-gray-600">
                      <span className="inline-flex items-center gap-2">
                        <span className="h-2 w-2 rounded-full bg-slate-800" /> Solved
                      </span>
                      <span className="inline-flex items-center gap-2">
                        <span className="h-2 w-2 rounded-full bg-amber-700" /> Unsolved
                      </span>
                    </div>
                  </div>

                  <div className="mt-4 h-4 rounded-full bg-slate-200 overflow-hidden">
                    <div className="h-full rounded-full bg-slate-800" style={{ width: `${station.solved}%` }} />
                  </div>
                  <div className="mt-2 h-4 rounded-full bg-slate-200 overflow-hidden">
                    <div className="h-full rounded-full bg-amber-700" style={{ width: `${station.unsolved}%` }} />
                  </div>
                </div>
              ))}
            </div>
